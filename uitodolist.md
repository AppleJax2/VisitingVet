...
*   **(P1) Refactor Dashboard Components (Partially Done -> In Progress):** Update dashboard-specific components (`PetOwnerDashboard`, `ProviderDashboard`, `ClinicDashboard`, `AdminLayout`, and their sub-components like `MyPetsPage`, `ProviderAppointmentsPage`, `AdminUserListPage`, etc.) to use React-Bootstrap components (`Table`, `Tabs`, `Accordion`, `Pagination`, `Badge`, `Alert`, `Modal`, `Form`) and CoreUI components where appropriate (e.g., `CCard`, `CWidgetStatsF`). Replace custom table styles, form layouts, and UI elements. Affects files in `src/components/Dashboard/`, `src/pages/Admin/`, specific role pages in `src/pages/`.
    *   **DONE:** `PetOwnerDashboard.jsx`, `ProviderDashboard.jsx`, `ClinicDashboard.jsx`, `MyPetsPage.jsx`, `ProviderAppointmentsPage.jsx`, `AdminUserListPage.jsx`
    *   **TODO:** Remaining dashboard/admin/role-specific components (e.g., `AdminDashboardPage`, `AdminVerificationListPage`, `AdminLogPage`, `AdminSettingsPage`, `AdminEditProfilePage`, `AdminResetPasswordPage`, `AdminUserDetailPage`, `AdminMFASetupPage`, `AdminSessionsPage`, `AdminPermissionsPage`, `AdminAnalyticsDashboardPage`, `UserProfilePage.jsx`, `MessagesPage.jsx`, `PaymentPage.jsx`, `PaymentSuccessPage.jsx`, `StripeConnectReturnPage.jsx`, `StripeConnectRefreshPage.jsx`). Verify if `AdminLayout` is still relevant.
